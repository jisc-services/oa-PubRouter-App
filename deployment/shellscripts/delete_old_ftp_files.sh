#!/bin/bash
#---------------------------------------------------------------------------------
# Deletes items that are older than specified number of days from FTP folders.
# NOTE that this deletes contents that are 2 levels down from specified directory.
#
# Params:
...
if [ $DAYS_TO_KEEP -ge $MIN_DAYS_TO_KEEP ]; then
    # Find all the directories 2 levels below $STORE_DIR that are older than $DAYS_TO_KEEP days
    ...
    COUNT_DELETED=`find $STORE_DIR -mindepth 2 -maxdepth 2 -type d -mtime +$DAYS_TO_KEEP -exec rm -rfv '{}' \; | wc -l`
