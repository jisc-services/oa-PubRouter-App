...
def _random_url():
    ...
    https://pubrouter.jisc.ac.uk/static/info_small.png
    ...
    return "https://pubrouter.jisc.ac.uk/static/info_small.png"
...
def _make_notification(error=False, routable=0, repo_configs=None):
    ...
    note = APIFactory.incoming_notification_dict(with_content=True)
    ...
    note["embargo"]["duration"] = (ee - es).total_seconds() // (60 * 60 * 24 * 30)
    ...
    note["metadata"]["funding"] = []
    for i in range(randint(1, 2)):
        funding = {}
        funding["name"] = _random_string(3, 6)
        funding["identifier"] = [{"type" : "ringold", "id" : _random_string(10, 16)}]
        funding["grant_numbers"] = _random_string(5, 7)
        note["metadata"]["funding"].append(funding)
    ...
    for k, v in uber.items():
        if k == "domains":
            ...
            # add a funding with that grant number
            for grant in v:
                funding = {}
                funding["name"] = _random_string(3, 6)
                funding["grant_numbers"] = [grant]
                note["metadata"]["funding"].append(funding)
        elif k == "strings":
...
def _get_file_path(parent_dir, max_file_size, error=False):
    ...
    PackageFactory.make_custom_zip(path, invalid_jats=invalid_jats, corrupt_zip=corrupt_zip, target_size=size)
    return path
    ...
    while True:
        try:
            ...
            try:
                ...
                app.logger.warning("Thread:{x} - Validate request resulted in expected exception".format(x=tname))
...
def create(base_url, keys, throttle, mdrate, mderrors, cterrors, max_file_size, tmpdir, retrieve_rate, routable, repo_configs):
    ...
    while True:
        try:
            ...
            id = None
            try:
                id, loc = j.create_unrouted_note(note, file_handle)
    ...
    while True:
        try:
            ...
            if err == "err":
                ...
                url = j.make_url("routed", id=repository_id, params=params)
            ...
            try:
                count = 0
                for note in j.iterate_routed_outgoing_notes(since, repository_id, page_size):
