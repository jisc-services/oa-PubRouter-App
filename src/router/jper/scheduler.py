...
def _process_unrouted(harvested):
    ...
    try:
        ...
        # returns a list of unrouted notification DICTs from the last ~three months
        unrouted_scroller = unrouted_class.unrouted_scroller_obj()
        with unrouted_scroller:
            for notification_dict in unrouted_scroller:
                if proc_unrouted_killer.killed:
                    break
                routed_bool = False
                try:
                    note_id = notification_dict["id"]
                    notification = unrouted_class(notification_dict)
                    note_count += 1
