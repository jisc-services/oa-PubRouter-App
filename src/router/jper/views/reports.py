...
def adhoc_duplicate_submissions():
    ...
    if from_date or to_date:
        ...
        # earliest allowed date is 3 months ago (as notification data before that has been deleted from database)
        earliest_date_str = (today - relativedelta(months=3)).strftime('%Y-%m-%d')
        if to_date:
            if to_date < earliest_date_str or to_date > today_str:
                flash(f"The to-date must be in the range {earliest_date_str} to {today_str}", "error")
                to_date = today_str
                ok = False
        if from_date:
            if from_date < earliest_date_str:
                from_date = earliest_date_str
                flash(f"The earliest date has been set to 3 months ago ({from_date})", "warning")
            if not to_date:
                # 1 week after from_date
                to_date = (datetime.strptime(from_date, "%Y-%m-%d").date() + relativedelta(days=7)).strftime("%Y-%m-%d")
        else:
            # 1 week before to_date
            from_date = (datetime.strptime(to_date, "%Y-%m-%d").date() - relativedelta(days=7)).strftime("%Y-%m-%d")
            if from_date < earliest_date_str:
                from_date = earliest_date_str
        ...
        if ok:
            ...
            if display_on_screen or not os.path.exists(tmp_filepath):
                ...
                if not data_list:
                    flash("No duplicate submissions found in the specified date range.", "info")
