...
from router.shared.models.doi_register import describe_bit_settings, describe_differences
...
def content_ajax():
    ...
    *** WARNING *** - This function currently allows the input of potentially dangerous HTML or Javascript [JS] code
                   ...
                   to a limited number of trustworthy Jisc administrators.  This defect could be addressed by introducing
...
@blueprint.route("/note_details", methods=['GET'])
def note_details():
    """
    Display NOTIFICATION metrics.
    """
    note = orig_deposit_diffs = cum_deposit_diffs = metrics_dict = bits_on_list = bits_off_list = highlight_bits = None
    ...
    if note_id:
        ...
        if note is None:
            ...
            highlight_bits = 0
            if dup_diffs_list:
                orig_deposit_diffs = describe_differences(dup_diffs_list[0])    # returns {} if no differences
                cum_deposit_diffs = describe_differences(dup_diffs_list[1]) if len(dup_diffs_list) > 1 else None
                if cum_deposit_diffs:
                    highlight_bits = cum_deposit_diffs["add_bits"]
                elif orig_deposit_diffs:
                    highlight_bits = orig_deposit_diffs["add_bits"]


            metrics_dict = note.metrics     # Will be None if Notification has no DOI
            if metrics_dict:
                bits_on_list, bits_off_list = describe_bit_settings(metrics_dict["val"]["bit_field"], True, [2,1])

    ## orig_deposit_diffs will have 1 of 3 values:
    #   None -> No previous note with same DOI;
    #   {} -> No difference between this note & first note with same DOI;
    #   {...} -> Populated dict - describes differences between this note & first with same DOI
    ## cum_deposit_diffs will have 1 of 3 values:
    #   None -> No or 1 previous note with same DOI;
    #   {} -> No difference between this note & cumulative view of previous notes with same DOI;
    #   {...} -> Populated dict - describes differences between this note & cumulative view of previous notes with same DOI
    return render_template(
        "admin/note_details.html",
        title="Notification details",
        notification=note,
        orig_deposit_diffs=orig_deposit_diffs,
        cum_deposit_diffs=cum_deposit_diffs,
        metrics_dict=metrics_dict,
        bits_on_list=bits_on_list,
        bits_off_list=bits_off_list,
        format_note_fn=format_note_as_html,
