...
from router.shared.mysql_dao import UnroutedNotificationDAO, RoutedNotificationDAO, NotificationAccountDAO,\
    NotificationDAO, HarvestedUnroutedNotificationDAO, CONN_CLOSE, DICT, PURGE, DAOException
...
class UnroutedMixin:
    ...
    ** MUST BE USED in conjunction with ...DAO class. **
    ...
    def unrouted_scroller_obj(cls, order="asc"):
        """
        Return a basic Scroller object for scrolling through ALL unrouted notification DICTs.  The connection is
        automatically closed when all records have been scrolled through.

        Notes: 
          * DICTs (rather than UnroutedNotification or HarvestedNotification objects) are returned so that any failure
            to instantiate an Object can be caught in the scrolling loop (otherwise the scroller would be aborted).
          * We use a basic scroller here, unlike the Reusable-scroller used for routed_scroller_obj, because the
        ...
        """
        return cls.scroller_obj(pull_name="all", order_by=order, end_action=CONN_CLOSE, rec_format=DICT,
