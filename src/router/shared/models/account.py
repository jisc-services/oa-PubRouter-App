...
class AccOrg(dataobj.DataObj, AccOrgDAO, UserMixin):
    ...
    @staticmethod
    def org_name_to_filename(org_name, prefix=None, extension=None, lower=False):
        """
        Create an output filename from supplied Organisation Nme (including path if supplied)
        :param org_name: String - Organisation name
        :param prefix: String - String to be prefixed to the organisation name
        :param extension: String - Extension suffix like "json" or "csv"
        :param lower: Boolean - If true, return a lowercase filename
        ...
        filename = org_name.translate(org_translate_tbl)
        if prefix:
...
class AccUser(dataobj.DataObj, AccUserDAO, UserMixin):
    ...
    def pull_all_user_n_org_accounts(cls, org_types=None, live_test=None, role_codes=None, del_on_off=None, list_func=None):
        ...
        if rec_tuple_list:
            AccOrg.set_all_cols()  # Need to do this so recreate_json_dict_from_rec() works
...
class AccRepoMatchParams(dataobj.DataObj, AccRepoMatchParamsDAO):
    ...
    @staticmethod
    def csv_row_headings():
        """
        The heading row for CSV data generated by the function: csv_row_generator(...)
        """
        return ["Name Variants", "Domains", "Postcodes", "Organisation Identifiers", "Author Emails", "Grant Numbers", "ORCIDs"]

    @staticmethod
    def csv_row_generator(mp_dict, preserve_data=False):
        """
        Generates rows of data (a list of values) for outputting to a CSV file
        :param mp_dict: Matching parameters `matching_config` dict
        :param preserve_data: Boolean indicating whether to preserve the matching params data in the passed dict.
                              If False, then all matching parameters will be deleted from dict as they are processed.

        :return: rows of data: [name_variant, domain, postcode, org_id, email, grant, ORCID]
        """
        ...
        if mp_dict:
            if preserve_data:
                mp_dict = mp_dict.copy()

            # Create copies as the processing pops items from the lists
            name_vars = mp_dict.get("name_variants", [])
            domains = mp_dict.get("domains", [])
            postcodes = mp_dict.get("postcodes", [])
            org_ids = mp_dict.get("org_ids", [])
            emails = mp_dict.get("emails", [])
            grants = mp_dict.get("grants", [])
            orcids = mp_dict.get("orcids", [])

            # Important: the CSV rows headings (see csv_row_headings function above) must correspond to the order
            # in which data is added to list
            while True:
                next_row = []
