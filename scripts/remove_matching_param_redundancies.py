...
with app.app_context():
    for match_params in AccRepoMatchParams.pull_all(for_update=True):
        # remove redundant match params
        redundancy_messages = match_params.remove_redundant_matching_params_and_sort()
        if redundancy_messages:
            # print list of redundancies together with which repository they came from
            print(f"*** Repository ID: {match_params.id}")
